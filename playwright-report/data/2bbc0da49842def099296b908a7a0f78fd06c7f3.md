...
- Name: login.spec.ts >> QBank Login Tests @regression >> TC-03: Should not login with empty username
...
Call log:
  - waiting for locator('input[name="username"], input[id="username"], input[placeholder*="username" i], input[type="text"]').first() to be visible
    62 × locator resolved to hidden <input type="text" id="qb-corporate-id" ng-model="companyId" placeholder="Company ID" ng-hide="mainCtrl.isPersonalBanking" class="qb-signin-input ng-pristine ng-untouched ng-valid ng-empty ng-hide"/>
