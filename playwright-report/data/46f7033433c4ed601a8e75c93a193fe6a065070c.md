...
- Name: login.spec.ts >> QBank Login Tests @regression >> TC-04: Should not login with empty password
- Location: tests\login.spec.ts:32:7
...
Call log:
  - waiting for locator('input[name="username"], input[id="username"], input[placeholder*="username" i], input[type="text"]').first() to be visible
    59 × locator resolved to hidden <input type="text" id="qb-corporate-id" ng-model="companyId" placeholder="Company ID" ng-hide="mainCtrl.isPersonalBanking" class="qb-signin-input ng-pristine ng-untouched ng-valid ng-empty ng-hide"/>
